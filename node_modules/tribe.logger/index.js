...
var api = module.exports = {
    ...
    silly: function (message) {
        if (level >= 5) console.log(colorise('SILLY: ' + message, 'grey'));
    },
    debug: function (message) {
        if (level >= 4) console.log(colorise('DEBUG: ' + message, 'grey'));
    },
    info: function (message) {
        if (level >= 3) console.info(colorise('INFO: ' + message, 'white'));
    },
    warn: function (message) {
        if (level >= 2) console.warn(colorise('WARN: ' + message, 'red'));
    },
    error: function (message, error) {
        if (level >= 1)
            if(error)
                ...
                console.error(colorise('ERROR: ' + message, 'red'));
    },
