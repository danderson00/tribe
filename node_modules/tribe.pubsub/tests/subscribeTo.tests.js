suite('tribe.pubsub.subscribe.to', function () {
    ...
    test("subscriptions with property equal filters call subscribe with four arguments", function () {
        var pubsub = mockPubsub();
        subscribeTo(pubsub)('test').when('property').equals('value').execute('func');
        expect(pubsub.subscribe.callCount).to.equal(1);
        expect(pubsub.subscribe.firstCall.args).to.deep.equal(['test', 'func', 'property', { property: 'property', value: 'value' }]);
    });
