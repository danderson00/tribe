suite('tribe.pubsub.filters.expression', function () {
    ...
    test("multiple expressions", function () {
        ...
        list.add(1, 1, { p: 'data.p1', v: 1 });
        list.add(2, 2, [{ p: 'data.p1', v: 1 }, { p: 'data.p2', v: 1 }]);
        list.add(3, 3, [{ p: 'data.p1', v: 1 }, { p: 'data.p2', v: 2 }]);

        expect(list.get({ data: { p1: 1 } })).to.deep.equal([1]);
        expect(list.get({ data: { p1: 1, p2: 1 } })).to.deep.equal([1, 2]);
        expect(list.get({ data: { p1: 1, p2: 2 } })).to.deep.equal([1, 3]);
    });
});