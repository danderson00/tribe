suite('tribe.pubsub.filters.expression', function () {
    ...
    test("envelopes with multiple values for expressions can trigger multiple handlers", function () {
        var list = new filter();
        list.add(1, 1, [{ p: 'data.test', v: 1 }]);
        list.add(2, 2, [{ p: 'data.test', v: 2 }]);
        expect(list.get({ data: { test: [1, 2] } })).to.deep.equal([1, 2]);
    })
});
