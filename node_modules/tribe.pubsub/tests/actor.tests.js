suite('tribe.pubsub.actor', function () {
    ...
    test("actor only handles messages with mapped id property", function () {
        var spy1 = sinon.spy(),
            spy2 = sinon.spy(),
            actor1 = new actorModule(pubsub, { pubsub: pubsub, handles: { 'testTopic': spy1 } }, 1),
            actor2 = new actorModule(pubsub, { pubsub: pubsub, handles: { 'testTopic': spy2 } }, 2);

        actor1.mapsMessagePropertyToId('test');
        actor1.start();
        actor2.mapsMessagePropertyToId('test');
        actor2.start();
    ...
    test("additional properties are attached to actor", function () {
        var actor = new actorModule(pubsub, constructor, null, { p1: 1, p2: 'test' });

        function constructor(actor) {
            expect(actor.p1).to.equal(1);
            expect(actor.p2).to.equal('test');
        }
    });
