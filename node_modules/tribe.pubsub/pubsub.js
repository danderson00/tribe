...
var pubsub = module.exports = function (instanceOptions) {
    ...
    this.subscribe = function (topic, func, filter) {
        filter = filter || {};
        if (typeof (topic) === "string")
            return subscribers.add(topic, func, filter.name, filter.options);
        else if (utils.isArray(topic))
            return utils.map(topic, function(topicName) {
                return subscribers.add(topicName, func, filter.name, filter.options);
            });
        else
            return utils.map(topic, function (individualFunc, topicName) {
                return subscribers.add(topicName, individualFunc, filter.name, filter.options);
            });
    };
