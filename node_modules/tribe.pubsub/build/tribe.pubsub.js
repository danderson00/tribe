...
module.exports = function (handler, topic) {
    ...
    if (!handler)
        ...
        this.pubsub.subscribe(topic, messageHandlerFor(handler), this.filterName, this.filterOptions);
    else
        this.pubsub.subscribe(topic, childHandlerFor(handler), this.filterName, this.filterOptions);
...
// but then there is the overhead of traversing in prototype chain instead...
...
actor.prototype.mapsIdToMessageProperty = function (property) {
    this.filterName = 'property';
    this.filterOptions = { property: property, value: this.id };
};
...
module.exports = function (topic) {
    ...
    return {
        add: function (handler, token, filterOptions) {
        ...
        get: function (envelope) {
            ...
            for (var property in properties)
                if (properties.hasOwnProperty(property)) {
                    var value = envelope.data[property];
                    if(value !== undefined)
...
var lifetime = module.exports = function (parent, owner) {
    ...
    this.channel = function(channelId) {
        return new channel(self, channelId);
    };
    ...
    this.startActor = function (definition, data) {
        return new actor(self, definition).start(data);
    };
...
module.exports = function (instanceOptions) {
    ...
    this.subscribe = function (topic, func, filter, options) {
        if (typeof (topic) === "string")
            return subscribers.add(topic, func, filter, options);
        else if (utils.isArray(topic))
            return utils.map(topic, function(topicName) {
                return subscribers.add(topicName, func, filter, options);
            });
        else
            return utils.map(topic, function (individualFunc, topicName) {
                return subscribers.add(topicName, individualFunc, filter, options);
            });
    };
...
module.exports = function () {
    ...
    this.add = function (topic, handler, filter, filterOptions) {
        ...
        filters[filter].add(handler, token, filterOptions);
        return token;
    };
