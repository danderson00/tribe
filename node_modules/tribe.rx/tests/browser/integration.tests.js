...
suite('tribe.rx.browser.integration', function () {
    ...
    test("foreach is updated on new messages", function () {
        ...
        return node.pane.is.rendered.then(function () {
            node.pane.pubsub.publish('topic', { done: true, actorId: id, value: 'one' })
            node.pane.pubsub.publish('topic', { done: true, actorId: id, value: 'two' })
            node.pane.pubsub.publish('topic', { actorId: id, value: 'three' })
    ...
    test("topic filters messages by topic", function () {
        ...
        var id = uuid.v4(),
            node = T.createNode('body', { path: '/rx/topic', scope: { actorId: id } })
