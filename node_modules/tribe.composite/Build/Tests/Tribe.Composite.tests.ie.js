...
(function() {
    ...
    test("cloneData is recursive", function () {
        var result = utils.cloneData({
            parent: ko.observable({
                child: ko.observable('test')
            })
        });

        equal(result.parent.child, 'test');
    });

    test("cloneData recurses through arrays", function () {
        var result = utils.cloneData({
        ...
        equal(result.array.constructor, Array);
        equal(result.array.length, 2);
        equal(result.array[0], 'test');
        equal(result.array[1], 'test2');
    });
...
// Unit/Types/StringCollection.tests.js
(function () {
...
(function () {
    ...
    test("registerFacet takes path from T.scriptEnvironment", function () {
        var constructor = function () { };
        T.scriptEnvironment = { resourcePath: 'test' };
        T.registerFacet(constructor);
        equal(Test.Integration.context.facets.test.constructor, constructor);
    });
})();
...
// Integration/Scope.tests.js
module('Integration.Scope', { setup: Test.Integration.pubsubAsTribe });
// something weird happens with these tests - they pass in isolation
// running one after another causes the model for Scope/child pane
// to not get loaded the second time
...
test("scope cascades through panes", function () {
    ...
    equal($('#count').text(), '0');
    equal($('#resetScopeTotalCount').text(), '0');

    pubsub.publish('topic', { value2: 'test2' });
    equal($('#count').text(), '0');
    equal($('#resetScopeTotalCount').text(), '1');

    pubsub.publish('topic', { value: 'test', value2: 'test2' });
    equal($('#count').text(), '1');
    equal($('#resetTotalCount').text(), '4');
    equal($('#resetScopeTotalCount').text(), '2');
});
...
// Integration/Events/paneRendered.tests.js
(function() {
