suite('tribe.storage.sqlite3.queries', function () {
    ...
    test("store with no indexes", function () {
        ...
        expect(query[0].sql).to.equal("insert into messages values (?)");
        expect(query[0].parameters).to.deep.equal([JSON.stringify(envelope)]);
    });
    ...
    test("store with single index", function () {
        var envelope = { id: 1, topic: 'test', value1: 1 },
            ...
            expect(query[0].sql).to.equal("insert into messages values (?)");
            expect(query[0].parameters).to.deep.equal([JSON.stringify(envelope)]);
            expect(query[1].sql).to.equal("insert into messages_value1 (id, value) values (last_insert_rowid(), ?)");
            expect(query[1].parameters).to.deep.equal([1]);
    });
    ...
    test("retrieve with index", function () {
        ...
        expect(query.sql).to.equal('SELECT messages.rowid as __rowid, * FROM messages INNER JOIN messages_testId ON messages.rowid = messages_testId.id WHERE messages_testId.value = ? ORDER BY messages_testId.value');
    ...
    test("retrieve with array indexValue", function () {
        ...
        expect(query.sql).to.equal('SELECT messages.rowid as __rowid, * FROM messages INNER JOIN messages_testId ON messages.rowid = messages_testId.id WHERE messages_testId.value IN (?,?) ORDER BY messages_testId.value');
    ...
    test("retrieve with multiple indexes", function () {
        ...
        expect(query.sql).to.equal('SELECT messages.rowid as __rowid, * FROM messages INNER JOIN messages_testId ON messages.rowid = messages_testId.id INNER JOIN messages_id ON messages.rowid = messages_id.id WHERE messages_testId.value = ? AND messages_id.value >= ? ORDER BY messages_testId.value,messages_id.value');
