        ...
        io.sockets.on('connection', function (socket) {
            ...
            socket.on('subscribe', function (options, ack) {
                ...
                if (options.actor) {
                    ...
                    subscribe(actor.definition.topics, actor.definition.messageFilter, options.id, true);
                } else if (options.topics)
                    subscribe(options.topics, options.filter, options.value, options.replay);

                function subscribe(topics, filter, value, replay) {
                    ...
                    if (filter && replay)
                        return eventStore.retrieve({
                            ...
                            ack({ path: options.actor, id: options.id, envelopes: envelopes });
                        })
                        .fail(function (err) {
