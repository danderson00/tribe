...
module.exports = function (options, context) {
    ...
    var definition = actors.definition(options.actor),
        expression = expressions.apply(definition.expression, prepareScope(options.scope));
    ...
    function subscribe(topics, expression) {
        context.pubsub.subscribe(topics, function (data, envelope) {
            if (envelope.sourceId !== context.clientId && !envelope.hasBeenBroadcastTo(context.clientId)) {
                context.socket.emit('message', envelope);
                envelope.addToBroadcastList(context.clientId);
            }
        }, expression);
    }
