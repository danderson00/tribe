...
<html>
    <head>
        ...
        <% if(options.test.browser.loadApp) { %>
        ...
        <%   for(var list = _.pluck(context.dependencies, 'appPath').sort(), i = 0, l = list && list.length; i < l; i++) { %>
        <script type="text/javascript" src="<%= list[i] %>"></script>
        <%   } %>

        <%   for(var file in context.modules) { if (context.hasOwnProperty(file)) { %>
        <script type="text/javascript" src="modules/<%= file %>"></script>
        <%   } } %>

        <script type="text/javascript" src="app.js"></script>
        <link rel="stylesheet" type="text/css" href="app.css" />

        <%   for(var i = 0, l = context.html && context.html.length; i < l; i++) { %>
        ...
        <script type="text/javascript" src="tests.agent.js"></script>
    
        <script type="text/javascript">
            window.onload = function() {
                require('tribe/options').apply(<%=data.options%>);
                require('tribe.pubsub').channel('__test').connect();
