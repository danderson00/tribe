...
<% if (data.mobile) { %>
<body class="__mobile"><div data-bind="pane: { path: '<%= options.mobileStartPane %>', handlesNavigation: { transition: 'slideLeft' } }"></div></body>
<% } else { %>
<body><div data-bind="pane: { path: '<%= options.startPane %>', handlesNavigation: { transition: 'fade' } }"></div></body>
<% } %>
</html>
