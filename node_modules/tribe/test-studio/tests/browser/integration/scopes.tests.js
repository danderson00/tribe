suite('tribe.browser.integration.scopes', function () {
    ...
    // the problem here is not the second publish - it does not propagate to the server
    // it's the whole storeUnpublished thing - because we're doing this synchronously,
    // the first publish hasn't been acknowledged by the time we request the scope the
    // second time, so it returns the "unpublished" message as well as returning the same 
    ...
    test("server releases client scopes", function (done) {
        var scope = { facetId: uuid() },
            received;

        scopes.request(scope)
            .then(function () {
                return scopes.release(scope);
            })
            .then(function () {
                pubsub.subscribe('topic', function () {
                    received = true;
                })
                return gateway.publish({ topic: 'topic', data: scope, echo: true });
            })
            .then(function () {
                // give it a few ms for the message to come back
                setTimeout(function () {
                    done(received && new Error('message received after scope released'));
                }, 1000);
            });
    });
