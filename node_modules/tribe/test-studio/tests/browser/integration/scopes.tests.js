suite('tribe.browser.integration.scopes', function () {
    ...
    test("only server messages with seq later than requested are returned", function () {
        ...
        return eventStore.clear()
            ...
            .then(function () {
                return gateway.scope({ scope: scope, since: first.seq });
            })
    ...
    test("messages can participate in multiple scopes", function () {
        ...
        return eventStore.clear()
            .then(function () {
                return gateway.publish({ topic: 'topic', data: { actorId: [ids[0], ids[1]] } })
            })
            .then(function () {
                return gateway.publish({ topic: 'topic', data: { actorId: [ids[1], ids[2]] } })
            })
            .then(function () {
                return gateway.scope({ scope: { actorId: ids[0] } });
            })
            .then(function (result) {
                expect(result.envelopes.length).to.equal(1);
            })
            .then(function () {
                return gateway.scope({ scope: { actorId: ids[1] } });
            })
            .then(function (result) {
