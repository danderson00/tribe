suite('tribe.browser.integration.scopes', function () {
    ...
    test("messages can participate in multiple scopes", function () {
        ...
        return eventStore.clear()
            .then(function () {
                return hub.publish({ topic: 'topic', data: { actorId: [ids[0], ids[1]] } })
            })
            .then(function () {
                return hub.publish({ topic: 'topic', data: { actorId: [ids[1], ids[2]] } })
            })
            .then(function () {
                return hub.scope({ scope: { actorId: ids[0] } });
            })
            .then(function (result) {
                expect(result.envelopes.length).to.equal(1);
            })
            .then(function () {
                return hub.scope({ scope: { actorId: ids[1] } });
            })
            .then(function (result) {
