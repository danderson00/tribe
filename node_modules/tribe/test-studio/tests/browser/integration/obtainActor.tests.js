var pubsub = require('tribe.pubsub'),
    gateway = require('tribe/client/gateway'),
    uuid = require('node-uuid'),
    pubsub;

// this effectively integration tests restoreActor as well, but there probably should be some specific tests
suite('tribe.browser.integration.obtainActor', function () {
    test("actors receieve correct number of messages and are released correctly", function () {
        var actorId = uuid.v4();

        return pubsub.obtainActor('tests/obtain', { actorId: actorId })
            .then(function (actor) {
                pubsub.publish('topic', { actorId: actorId });
                expect(actor.count).to.equal(1);
                pubsub.releaseActor(actor);
                return gateway.publish({ topic: 'topic', data: { actorId: actorId } });
            })
            .then(function () {
                return pubsub.obtainActor('tests/obtain', { actorId: actorId });
            })
            .then(function (actor) {
                expect(actor.count).to.equal(2);
            });
    });

    test("actor with single property scope can be obtained with a scalar", function () {
        var actorId = uuid.v4();

        return pubsub.obtainActor('tests/obtain', actorId)
            .then(function (actor) {
                pubsub.publish('topic', { actorId: actorId });
                expect(actor.count).to.equal(1);
                pubsub.releaseActor(actor);
                return gateway.publish({ topic: 'topic', data: { actorId: actorId } });
            })
            .then(function () {
                return pubsub.obtainActor('tests/obtain', actorId);
            })
            .then(function (actor) {
                expect(actor.count).to.equal(2);
            });
    });

    test("actor inherits scope from pubsub scope", function () {
        var actorId = uuid.v4();
        pubsub = pubsub.createLifetime({ actorId: actorId });

        return pubsub.obtainActor('tests/obtain')
            .then(function (actor) {
                pubsub.publish('topic', { actorId: actorId });
                pubsub.publish('topic', { actorId: 'otherId' });
                expect(actor.count).to.equal(1);
                pubsub.releaseActor(actor);
                return gateway.publish({ topic: 'topic', data: { actorId: actorId } }).then(function () {
                    return gateway.publish({ topic: 'topic', data: { actorId: 'otherId' } });
                });
            })
            .then(function () {
                return pubsub.obtainActor('tests/obtain', actorId);
            })
            .then(function (actor) {
                expect(actor.count).to.equal(2);
            });
    });

    test("actors do not republish messages when obtaining", function () {
        var actorId = uuid.v4(),
            spy = sinon.spy();

        // The other tests fail when using a fresh pubsub, I don't know why and I couldn't be bothered to find out right now
        pubsub = new (require('tribe.pubsub/pubsub'))({ sync: true });

        pubsub.subscribe('fires', spy);

        return pubsub.obtainActor('tests/republish', { actorId: actorId })
            .then(function (actor) {
                actor.__actor.pubsub.publish('trigger', { actorId: actorId });
                expect(spy.callCount).to.equal(1);
                pubsub.releaseActor(actor);
                return pubsub.obtainActor('tests/republish', { actorId: actorId });
            })
            .then(function (actor) {
                expect(spy.callCount).to.equal(1);
            });
    });
});
