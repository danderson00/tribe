suite('tribe.browser.integration.flow.startChild', function () {
    ...
    test("corresponding facet is started if specified", function (done) {
        var pubsub = require('tribe.pubsub'),
            node = T.appendNode('body', { path: '/tests/childFlow', pubsub: pubsub })

        setTimeout(function () {
            pubsub.publish('flowWithFacet')

            setTimeout(function () {
                var spy = sinon.spy();
                pubsub.subscribe('flowWithFacet.response', spy)
                pubsub.publishSync('flowWithFacet.trigger')
                expect(spy.callCount).to.equal(1)

                // navigate away
                pubsub.publish('topic')
                setTimeout(function () {
                    // publish again, make sure facet is cleaned up
                    pubsub.publishSync('flowWithFacet.trigger')
                    expect(spy.callCount).to.equal(1)
                    done()
                }, 20)
