suite('tribe.browser.integration.flow.startChild', function () {
    ...
    test("corresponding actor is started if specified", function (done) {
        var pubsub = require('tribe.pubsub'),
            node = T.appendNode('body', { path: '/tests/childFlow', pubsub: pubsub })

        setTimeout(function () {
            pubsub.publish('flowWithActor')

            setTimeout(function () {
                var spy = sinon.spy();
                pubsub.subscribe('flowWithActor.response', spy)
                pubsub.publishSync('flowWithActor.trigger')
                expect(spy.callCount).to.equal(1)

                // navigate away
                pubsub.publish('topic')
                setTimeout(function () {
                    // publish again, make sure actor is cleaned up
                    pubsub.publishSync('flowWithActor.trigger')
                    expect(spy.callCount).to.equal(1)
                    done()
                }, 20)
