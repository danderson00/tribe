ko.bindingHandlers.arrayField = {
    init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
        ...
        length.subscribe(function (newLength) {
            ...
            for(var i = 0; i < Math.abs(difference); i++)
                if(difference > 0) {
                    ...
                    boundValue.push({ value: newValue, index: boundValue().length });
                    value.push(newValue);
                } else {
                    boundValue.pop();
                    value.pop();
        ...
        function createArray() {
            return ko.observableArray(Array.apply(null, Array(+length())).map(function (value, index) { return { value: ko.observable(), index: index } }));
        }
