    ...
    return loadDependencies(this, definition, scope)
        .then(function (dependencies) {
            ...
            if (actor.metadata.isDistributed)
                return self.syncActor(actor, scope)
...
pubsub.prototype.syncActor = function (actor, scope) {
...
pubsub.prototype.releaseActor = function (actorOrScope) {
    var scope = actorOrScope.__actor
        ? actorOrScope.__actor.scope
        : (actorOrScope.hasOwnProperty('scope') ? actorOrScope.scope : actorOrScope)
    scope = utils.extend({}, this.scope, scope)
    scopes.release(scope);
};

lifetime.prototype.obtainActor = pubsub.prototype.obtainActor;
lifetime.prototype.syncActor = pubsub.prototype.syncActor;
...
pubsub.prototype.startActor = function (path, scope, options) {
    ...
    function attachToHub(path, actor, replay) {
        ...
        return gateway.actor({ actor: path, scope: actor.scope, replay: replay });
    }
};
