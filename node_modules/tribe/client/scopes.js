...
module.exports = {
        ...
        function requestScope(since) {
            ...
            data.promise = Q.when(gateway.scope.request({ scope: scope, since: since }))
                .then(function (result) {
                    ...
                    data.serverToken = result && result.token; 
    ...
    release: function (scope) {
        ...
        data.count--;
        log.silly("Decremented count for " + jsonScope + " to " + data.count);

        if(data.count < 0) {
            log.warn("POTENTIAL MEMORY LEAK: scope " + JSON.stringify(scope) + " had count " + data.count);
            data.count = 0;
        }

        if(!data.count && data.token) {
            gateway.scope.release({ scope: scope, token: data.serverToken });
            pubsub.unsubscribe(data.token);
            removeScopeData(jsonScope);
        }
    },
...
function removeScopeData(key) {
    if(scopes[key])
        scopes[key] = undefined;
}
