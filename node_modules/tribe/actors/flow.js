            ...
            return {
                ...
                startChild: function (path, options) {
                    ...
                    actor.handles(topic, function (data) {
                        ...
                        options.scope = options.scope || data
                        actor.startChild(path, options)
                    })
                }
            }
        }

        actor.startChild = function (path, options) {
            options = options || {};
            if(options.actor === true) options.actor = path
            pane({ path: '/__flow', data: { flow: path, transition: options.transition, scope: options.scope, actor: options.actor } })
        }
