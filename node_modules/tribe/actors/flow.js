    ...
    return function (actor) {
        ...
        actor.on = function (topic) {
            return {
                ...
                startChild: function (path, transition) {
                    actor.handles(topic, function () {
                        actor.startChild(path, transition)
                    })
                }
            }
        }

        actor.startChild = function (path, transition) {
            pane({ path: '/__flow', data: { flow: path, transition: transition } })
        }
