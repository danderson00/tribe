suite('tribe.eventStore.sqlite3.queries', function () {
    ...
    test("retrieve with array indexValue", function () {
        var query = queries.retrieve({ indexName: 'testId', indexValue: [1, 2] });
        expect(query.query).to.equal('select * from messages where testId in (?,?)');
        expect(query.params).to.deep.equal([1, 2]);
    });
