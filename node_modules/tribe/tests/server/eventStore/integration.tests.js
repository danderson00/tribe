...
createSuite('sqlite3', { filename: ':memory:' });

function createSuite(storeName, options) {
    var store = require('tribe/eventStore');

    suite('tribe.eventStore.integration.' + storeName, function () {
        // this is really basic, probably requires more comprehensive testing
        test("basic store and retrieve", function () {
            store.initialise(_.extend({ name: storeName, indexes: ['test1', 'test2'] }, options))
            .then(function () {
                return Q.all(
                ...
                expect(envelope[0]).to.deep.equal({ topic: 'topic', data: { test1: 1, test2: 'a' }, id: 1 });
                expect(envelope[1]).to.deep.equal({ topic: 'topic', data: { test1: 1, test2: 'b' }, test3: 'test', id: 4 });
            });
        });
    });
}