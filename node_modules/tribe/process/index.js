...
module.exports = {
    start: function (processOptions) {
        ...
        function publishChildMessage(envelope) {
            envelope.channelId = processOptions.channel;
            pubsub.publish(envelope);
        }

        function publishMessageToChild(data, envelope) {
            if (child && child.connected && envelope.channelId === processOptions.channel)
                child.send(utils.messages.prepareEnvelope(envelope));
        }
