...
suite('tribe.build.integration', function () {
    test("app is built", function () {
        // this could impact later tests?
        options.basePath = path.resolve(__dirname, '../../files/build/') + '\\';
        options.debug = true;

        context = {};

        require.refresh(/\.\/activities\//);
        ...
        build.configure({
            ...
            tasks: [
                ...
                { activity: 'scripts', options: { path: path.resolve(options.modulePath, 'client') } }
            ]
        });
        return build.execute(context)
            .then(function () {
                ...
                expect(js).to.have.string("T.scriptEnvironment = { resourcePath: '/pane' };\n");    //build/blocks/browserify.js
                expect(js).to.have.string('saga: T.registerSaga');                                  //register.client.js
                expect(js).to.have.string('//# sourceMappingURL=');                                 //build/blocks/browserify.js
                expect(js).to.have.string('var hub = module.exports');                              //client/hub.js
                expect(js).to.have.string('Tribe = {}');                                            //client/dependencies/Tribe.debug.js
...
function output(file) {
    return memory.collections['build'][file];
}
