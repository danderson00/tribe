// this is less than ideal. It allows code running on the server side like sagas to use knockout without requiring it or even installing it
// possibly will be replaced by passing knockout to modules using a custom module loader like tests are now
ko = require('knockout');

module.exports = {
    // server
    start: function (options) {
        options = require('tribe/options').apply(options);

        require('tribe/build')
            .configure(options.build)
            .execute();

        if (options.debug)
            require('tribe/process/inspector').start(options.inspectorPort);

        require('tribe/test').initialise(true);
        require('tribe/server/http').start();
        require('tribe/server/socket').start();
    },

    // common
    register: require('tribe/register'),
    pubsub: require('tribe.pubsub')
};